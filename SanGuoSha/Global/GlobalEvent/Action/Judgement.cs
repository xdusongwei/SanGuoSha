    ...
    public partial class GlobalEvent
    {
        ...
        private void Judgement(ChiefBase aChief, GlobalData aData)
        {
            ...
            while (GamePlayers[aChief].Debuff.Count != 0 && !GamePlayers[aChief].Dead)
            {
                ...
                //加入子事件队列
                queRecoard.Enqueue(new EventRecoard(aChief, aChief, [buff], buff.CardEffect, string.Empty));
                lstCardBin.Add(buff);
                //判断Debuff类型
                switch (buff.CardEffect)
                {
                    ...
                    case Card.Effect.BingLiangCunDuan:
                        ...
                        AsynchronousCore.SendMessage(
                            new Beaver("removedebuff", aChief.ChiefName, Card.Cards2Beaver("cards", [buff])).ToString());
                            //new XElement("removedebuff",
                            //    new XElement("target", aChief.ChiefName),
                            //    Card.Cards2XML("cards", new Card[] { buff })
                            //));
                        DropCards(true, CardFrom.JudgementCard, string.Empty, [judgementBLCD], Card.Effect.None, aChief, aChief, null);
                        break;
                    //乐不思蜀 
                    case Card.Effect.LeBuSiShu:
                        ...
                        AsynchronousCore.SendMessage(
                            ...
                            //    Card.Cards2XML("cards", new Card[] { buff })
                            //));
                        DropCards(true, CardFrom.JudgementCard, string.Empty, [judgementLBSS], Card.Effect.None, aChief, aChief, null);
                        break;
                    ...
                    case Card.Effect.ShanDian:
                            ...
                            if (next != aChief)
                                do
                                {
                                    ...
                                    if (!GamePlayers[next].HasDebuff(Card.Effect.ShanDian))
                                    {
                                        ...
                                        AsynchronousCore.SendMessage(
                                            new Beaver("sd.moved", aChief.ChiefName, next.ChiefName, Card.Cards2Beaver("cards", [buff])).ToString());
                        ...
                        if (judgementSD.CardSuit == Card.Suit.Spade && judgementSD.CardIndex > 1 && judgementSD.CardIndex < 10)
                        {
                            ...
                            if (GamePlayers[aChief].Armor != null)
                                DamageHealth(aChief, Armor.CalcDamage(3, [buff], GamePlayers[aChief].Armor.CardEffect), null, new EventRecoard(null, aChief, null, [buff], Card.Effect.ShanDian, string.Empty));
                            else
                                DamageHealth(aChief, 3, null, new EventRecoard(null, aChief, null, [buff], Card.Effect.ShanDian, string.Empty));
                        }
                        else
                        {
                            //没有费血,把debuff牌从垃圾桶里拣出来
                        ...
                        //判定牌加入子事件节点
                        DropCards(true, CardFrom.JudgementCard, string.Empty, [judgementSD], Card.Effect.None, aChief, aChief, null);
                        break;
