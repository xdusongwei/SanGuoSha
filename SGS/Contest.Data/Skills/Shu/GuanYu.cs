...
namespace SGS.ServerCore.Contest.Data
{
    internal class SkillWuSheng : SkillBase
    {
        ...
        public override bool ActiveSkill(string aSkillName, Card[] aCards, ChiefBase aChief, ChiefBase[] aTargets, MessageCore.AskForEnum aAskFor, ref Card.Effect aEffect, GlobalData aData)
        {
            ...
            {
                if (SkillStatus == SkillEnabled.Enable && aCards.Count() == 1 && (aCards[0].CardHuaSe == Card.Suit.FangPian || aCards[0].CardHuaSe == Card.Suit.HongTao) && (aAskFor == MessageCore.AskForEnum.Sha || aAskFor == MessageCore.AskForEnum.Aggressive))
                {
